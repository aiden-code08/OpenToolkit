...
class Database {
    ...
    initialize() {
        const accounts = this.database.prepare(`CREATE TABLE IF NOT EXISTS accounts (id INT NOT NULL AUTOINCREMENT, username TEXT NOT NULL, password TEXT NOT NULL, token TEXT NOT NULL);`).run();
        const sessions = this.database.prepare(`CREATE TABLE IF NOT EXISTS sessions (sessionid TEXT NOT NULL, streamkey TEXT NOT NULL);`).run();
    }
    /**
     * ----------------------------------
     * Accounts
